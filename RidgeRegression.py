...
def create_function(m_dim, x, y):
    ...
    for i in range(n):
        ...
        phi.append(phi_i)
    phi = np.array(phi)
    pseudo_inv_phi = np.linalg.inv(np.dot(phi.T, phi) + 100*np.identity(phi.shape[1]))
    app_inv_phi = np.dot(phi, pseudo_inv_phi)
    app_inv_phi_1 = np.dot(app_inv_phi, phi.T)
    y_hat = np.dot(app_inv_phi_1, y)

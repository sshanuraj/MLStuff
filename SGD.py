...
def select_random_sample(X):
    N = X.shape[0]
    random_index = int(np.random.uniform(0, N))

    return np.array([X[random_index]]), random_index
...
def train(w, x, y, N_steps):
    ...
    for i in range(1, N_steps+1):
        ...
        x_random, random_index = select_random_sample(x)
        y_random = np.array([y[random_index]])
        grads = get_grads(x_random, y_random, w)
        w = update_params(grads, w, 0.002)
    return w
